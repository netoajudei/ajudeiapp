...
serve(async (req)=>{
  ...
  try {
    ...
    const supabaseStorageBase = (Deno.env.get('SUPABASE_URL') ?? '').replace(/\/$/, '');
    const cardapios = [
      {
        url: `${supabaseStorageBase}/storage/v1/object/public/varanda.italia//cardapio_almoco_var_italia_abril2025.pdf`,
        caption: "Cardápio do Almoço"
      },
      {
        url: `${supabaseStorageBase}/storage/v1/object/public/varanda.italia//cardapio_noite_var_italia_abril_2025.pdf`,
